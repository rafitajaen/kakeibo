...
public sealed class MemberLeftAuditHandlerTests
{
    [Fact]
    public async Task HandleAsync_MemberLeftEvent_RecordsAuditEntryWithCorrectFields()
    {
        var auditService = Substitute.For<IAuditService>();
        var handler = new MemberLeftAuditHandler(auditService);
        var walletId = Guid.NewGuid();
        var userId = Guid.NewGuid();
        var occurredAt = Instant.FromUtc(2026, 3, 1, 12, 0);

        var @event = new MemberLeftEvent
        {
            WalletId = walletId,
            UserId = userId,
        ...
        await auditService.Received(1).RecordAsync(
            Arg.Is<AuditEntry>(e =>
                e.UserId == userId &&
                e.Action == AuditActions.MemberLeft &&
                e.EntityType == "Wallet" &&
                e.EntityId == walletId &&
