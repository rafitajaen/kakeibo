...
test.describe("Goals", () => {
    ...
    // Run manually: bun run app:test:e2e --grep "goal"
    ...
    test("delete goal → disappears from list", async ({ page }) => {
        ...
        await page.getByTestId("goal-item").first().getByRole("button", { name: "Delete" }).click();
        // Confirm deletion via the AlertDialog action button
