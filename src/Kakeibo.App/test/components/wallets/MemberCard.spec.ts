...
function mountCard(member: WalletMember, currentUserId: string, isOwner: boolean) {
    return mount(MemberCard, {
        props: { member, currentUserId, isOwner },
        global: { plugins: [i18n] },
    });
}
...
describe("MemberCard", () => {
    ...
    it("does not show a remove button when a non-owner views another member", () => {
        const wrapper = mountCard(regularMember, "other-member-id", false);
        expect(wrapper.text()).not.toContain("Remove");
        expect(wrapper.text()).not.toContain("Leave wallet");
    });
